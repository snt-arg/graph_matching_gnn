#graph_matching.py
GNN_PATH = './GNN/'
# Fallback root for the real validation scenes. GNN/real_* is not present in every checkout,
# but the dataset dump always carries all three environments under real/real_{adj,fully}/.
import os
...
_REQUIRED_MODULES = ["torch", "torch_geometric", "sklearn", "pandas", "shapely", "seaborn",
                     "pygmtools", "matplotlib", "tensorboard", "optuna", "plotly", "wandb"]
...
def nx_to_pyg_data_preserve_order(graph: nx.DiGraph) -> Data:
    ...
    Node features are reduced to [type_onehot(2), length_or_-1(1)] -- 'center'
    and 'normal' are deliberately dropped (see arXiv:2409.11972 Sec III-B: "the initial node
    embedding is limited to its length"), since leaving raw frame-dependent position/orientation
    in node features preserves the exact contamination path diagnosed as the root cause of
    adj_glob_65's real-world embedding collapse. Relative geometry now lives entirely in
    `edge_attr` (see edge_features.py), computed from RAW (pre-normalization) center/normal.
    """
...
def build_real_validation_pairs(real_dir: str, verbose: bool = True,
    ...
    for folder in sorted(os.listdir(real_dir)):
        ...
        # edge_attr carries an unbounded d_ij column, so it needs the same treatment --
        # skipping it here would feed real scenes at a different scale than training.
...
in_dim = train_dataset[0][0].x.size(1)
edge_dim = train_dataset[0][0].edge_attr.size(1)  # fixed feature-engineering property, not searched -- like in_dim
learning_rate = best_params['lr']
weight_decay = best_params['weight_decay']
hidden_dim = best_params['hidden_dim']
out_dim = best_params['out_dim']
edge_hidden_dim = best_params['edge_hidden_dim']
...
# Logger W&B.
# Wrapped so a logging outage (no network, no credentials, WANDB_MODE=offline on a local box)
# degrades to "no W&B" instead of killing a multi-hour training run. Every wandb.log call in
# this file is already guarded by `if wandb.run is not None`, so a failed init is safe.
try:
    wandb.init(
    ...
    print(f"[WARNING] wandb.init failed ({_wandb_err}) -> continuing without W&B logging. "
          f"TensorBoard logging and checkpointing are unaffected.")
...
# GNN/real_* is not present in every checkout; the dataset dump always has all three
# environments (47_basement, 47_topfloor_modified, CF12). Falling back keeps real-scene F1
# from silently evaluating on an empty set, which is what happened before.
...
if _norm_stats is None:
    _real_mean = _real_std = _real_edge_mean = _real_edge_std = None
    ...
    _real_mean, _real_std, _real_edge_mean, _real_edge_std = _norm_stats
    if _real_edge_mean is None:
        print("[WARNING] norm_stats.pt has no edge_attr stats (dataset predates edge "
              "normalization) -> real-scene edge features stay unscaled. Rebuild with "
              "dataset_gen_adj_glob_edgefeat.py.")

print(f"\nLoading real validation set from {real_val_dir} "
      f"(normalized={_norm_stats is not None})...")
real_val_pairs = build_real_validation_pairs(real_val_dir,
                                             mean=_real_mean, std=_real_std,
                                             edge_mean=_real_edge_mean,
                                             edge_std=_real_edge_std)
