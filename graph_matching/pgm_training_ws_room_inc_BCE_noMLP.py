...
def train_loop(model, optimizer, train_loader, val_loader, num_epochs, writer,
               ...
               patience=10, resume=False, unfreeze_epoch=None):
...
###     PARTIAL GRAPH MATCHING MODEL without MLP
...
def objective_pgm(trial, train_dataset, val_dataset, path):
    ...
    # Flexible model for partial matching (no MLP)
...
models_path = os.path.join(GNN_PATH, 'models', "partial_graph_matching", "ws_room_dropout_noise_inc_BCE_noMLP")
...
model = MatchingModel_GATv2Sinkhorn(
    in_dim=in_dim,
    hidden_dim=hidden_dim,
    out_dim=out_dim,
    attention_dropout=attn_dropout,
...
writer = setup_tb_logger(
    ...
    dataset_name="PGM_ws_room_dropout_noise_inc_BCE_noMLP",
